...
public class SampleController implements Initializable {
	...
	ObservableList<String> video_extension_list = FXCollections.observableArrayList(Extension.MP4.name(),
			Extension.GP.name(), Extension.G2.name(), Extension.MKV.name(), Extension.AVI.name());
	ObservableList<String> audio_codec_list = FXCollections.observableArrayList();
	ObservableList<String> video_codec_list = FXCollections.observableArrayList();
	...
	public void ButtonBrowseVideoAction(ActionEvent event) {
		...
		if (video != null) {
			...
			slider_bitrate
					.setValue(Main.destination.resolution.width * Main.destination.resolution.height * 60 / 10000);
			text_bitrate.setText(String
	...
	public void ButtonBrowseAudioAction(ActionEvent event) {
		...
		fc.getExtensionFilters()
				.addAll(new ExtensionFilter("MP3, M4A, OGG, OGA, AAC", "*.mp3", "*.m4a", "*.ogg", "*.oga", "*.aac"));
	...
	public void ButtonBrowseSubtitleAction(ActionEvent event) {
		FileChooser fc = new FileChooser();
		fc.getExtensionFilters().addAll(new ExtensionFilter("SRT, MKS", "*.srt", "*.mks"));
	...
	public void checkCheckboxBitrate() {
		if (checkbox_bitrate.isSelected()) {
			if (Main.destination != null) {
				...
				text_bitrate.setText(String
						.valueOf(Main.destination.resolution.width * Main.destination.resolution.height * 60 / 10000));
				Main.destination.vbitrate = Main.destination.resolution.width * Main.destination.resolution.height * 60
						/ 10000;
				Main.destination.use_crf = false;
			} else {
				slider_bitrate.setValue(100);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		// Methode pour g�rer le scorll du bitrate audio
		...
		box_extension.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			...
			public void changed(ObservableValue<? extends Number> observableValue, Number old_value, Number new_value) {
				...
				case "MKV":
					box_video.setItems(FXCollections.observableArrayList(VCodec.LIBX264.name(), VCodec.LIBX265.name(),
							VCodec.LIBXVID.name()));
					box_audio.setItems(FXCollections.observableArrayList(ACodec.VORBIS.name(), ACodec.OPUS.name(),
							ACodec.MP3LAME.name(), ACodec.FLAC.name(), ACodec.AAC.name()));
