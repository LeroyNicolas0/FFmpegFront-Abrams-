	...
	public void scanffmpeg(String url) {
		...
		try {
			...
			while ((lineToParse = infoBuff.readLine()) != null) {
				...
				/*
				 * We look for a line that looks like this : Input #0, mov,mp4,m4a,3gp,3g2,mj2,
				 * from 'physics.mp4':
				 * 
				 */
				if (lineToParse.contains("Input #0")) {
					String extractedLine = lineToParse.substring(lineToParse.indexOf("from \'") + 6,
							lineToParse.indexOf("\':"));
				...
				/*
				 * We look for a line that looks like this : Duration: 00:00:11.02, start:
				 * 0.000000, bitrate: 9570 kb/s And we extract : 00:00:11:02
				 * 
				 */
				if (lineToParse.contains("Duration")) {
					System.out.println("Line parsed : " + lineToParse);
					String extractedLine = lineToParse.substring(lineToParse.indexOf("Duration: ") + 9,
							lineToParse.indexOf(","));
					System.out.println("Strings extracted : " + extractedLine);
					duration = Main.stringToTime(extractedLine);
				}
				/*
				 * Here we look for the following line : Stream #0:0(eng): Video: h264 (High)
				 * (avc1 / 0x31637661), yuvj420p(pc), 1920x1080, 1319 kb/s, 23.98 fps, 23.98
				 * tbr, 24k tbn, 47.95 tbc (default) And we extract : 1920x1080.
				 */
				...
				/*
				 * We look for the following line : Stream #0:1(und): Audio: aac (LC) (mp4a /
				 * 0x6134706D), 48000 Hz, stereo, fltp, 128 kb/s (default) And we extract
				 * 1,(und), 48000 and 128.
				 */
				...
				if (lineToParse.contains("Stream") && lineToParse.toLowerCase().contains("audio")) {
					...
					a++;
					n++;
					AudioTrack at = new AudioTrack(url,
							lineToParse.substring(lineToParse.indexOf("(") + 1, lineToParse.indexOf(")")), a, n);
					at.integrated = true;
					pistes_audio.add(at);
					...
					abitrate = Float.parseFloat(cutTheEndOfTheLine.substring(cutTheEndOfTheLine.lastIndexOf(',') + 1,
							cutTheEndOfTheLine.lastIndexOf(' ')));
					System.out.println("String extracted : " + abitrate);
					String Line = cutTheEndOfTheLine.substring(cutTheEndOfTheLine.indexOf(",") + 2,
							cutTheEndOfTheLine.indexOf("Hz") - 1);
				...
				if (lineToParse.contains("Stream") && lineToParse.toLowerCase().contains("Subtitle")) {
					...
					s++;
					n++;
					SubtitleTrack t = new SubtitleTrack(url,
							lineToParse.substring(lineToParse.indexOf("(") + 1, lineToParse.indexOf(")")), s, n);
					t.integrated = true;
					st.add(t);
				}
			}
